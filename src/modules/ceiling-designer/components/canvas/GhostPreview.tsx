...
export const GhostPreview = memo(function GhostPreview() {
  ...
  const geometry = useMemo(() => {
    ...
    if (isLightBarCatalogItem(draggedCatalogItem)) {
      ...
      return { type: 'box', args: [length, 0.20, 0.30] as [number, number, number] } // Tripled from 0.08, 0.12
    }
    if (isHubCatalogItem(draggedCatalogItem)) {
      return { type: 'cylinder', args: [0.4, 0.4, 0.25, 32] as [number, number, number, number] } // Tripled from 0.15, 0.15, 0.1
    }
    if (isConnectorCatalogItem(draggedCatalogItem)) {
      return { type: 'cylinder', args: [0.2, 0.2, 0.2, 16] as [number, number, number, number] } // Tripled from 0.075, 0.075, 0.08
    }
    return { type: 'box', args: [0.5, 0.1, 0.5] as [number, number, number] }
  }, [draggedCatalogItem])

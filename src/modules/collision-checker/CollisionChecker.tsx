...
export const CollisionChecker = () => {
	...
	useFrame(() => {
		...
		// If there is no manually selected wall, disallow moving/adding models.
		if (!selectedWall) {
			// restore paints and clear helpers
			scene.traverse(obj => {
				if (checkIfElementISModel(obj)) {
					paintObjectWithRestore(obj, new Color(), true)
				}
			...
			if (draggingModelRef.current) {
				draggingModelRef.current.userData.closestWall = null
				draggingModelRef.current.userData.potentialQuaternion = null
			}

			setIsModelPositionValid(false)
